...
//TODO: stavfel
//TODO: referera till figurer
//TODO: figur för bumpmap delen
//TODO: parallax correction
//TODO: MAYBE: issues rubriken
...
=== Cubemap Texture 
A cubemap consists of 6 textures that make up the 6 sides of the cube. There is
built-in support for working with cubemap textures in various contexts directly
in OpenGL.
...
texture and various buffers, such as a depth buffer. In our case it's useful
for capturing the scene from various persepcives and positions before the final
render. 

== Implementation Details 
Here we will go into more detail on how various techniques were implemented as
well as overall project structure.
...
=== Dependencies We used no additional dependencies besides those provided for
the lab assignments. We did however use OpenGL 4.6 in our shaders, because we
could
...
correction. Both of this techniques are covered in later sections of the
report.

#figure( image("images/prerendered_cubemap.png", width: 65%), caption:
  [Pre-rendered cubemap.])

=== Dynamic Cubemap 
Dynamic cubemaps work by giving each mirror it's own FBO, that contains a
cubemap texture. The scene can then be rendered from the mirrors position, in
the positive and negative x, y and z directions. The result of these renders is
saved in the cubemap texture of the mirror's associated FBO. This texture is
then used for sampling when rays are reflected on the mirrors surface as
depicted in the basic cubemap section. This does not solve the issue of
distored perspective, but it makes the various models and changes in the scene
visible in the mirrors reflections.

#figure( image("images/dynamic_cubemap.png", width: 65%), caption: [Dynamic
  cubemap.])
...
like those found in "fun houses" at amusement parks. A problem with this
technique is that bumps in a real mirror could cause reflections of the mirror
itself. Because of limitations with how the dynamic cubemaps are created, this
effect can't be replicated. The mirror will also never be able to reflect it
self since the actual mirror model is flat.
...
#figure( 
  ...
  caption: [Parallax corrected cubemap on the left, normal cubemap on the right.],) <parallax-comparison>
...
to solve this is to use two FBO:s per mirror, which are switch between each
frame. This will solve the problem by using the cubemap texture from the
previous frame when rendering the cubemap texture for the current frame. This
will cause each recursive reflection step to lag one frame behind, but after a
while the changes will propagate through the reflections.
...
// WARN: Should probably be revised heavily 
